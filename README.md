...
| `c4po-lint-audit-monthly` | 1st–5th of each month at 9:00 AM¹ | Audits the entire workspace — recursing into subdirectories and the independent repos under `repos/*` (generic rules only there) — against the lint rules in the root CLAUDE.md. Emails the result every run — violations grouped by rule section, or an all-clear if clean. Runs once per month (state tracked in `c4po/.claude/scheduled/state/`). |
...
| `c4po-backlog-burndown` | Wed at 12:59 AM and 6:09 AM (~10h and ~4h50m before the weekly usage-limit reset) | Spends the credits left before the weekly usage reset on backlog work: reorders every backlog (workspace `BACKLOG.md` plus each `repos/*/BACKLOG.md`) by priority, emails a consolidated prioritized plan, then implements items top-down — committing (never pushing) and checking items off as it goes — until the plan empties or credits run out. Fires twice because the 5-hour session limit caps one run's burn: the first run plans, emails, and burns until limited; the second resumes the same plan with a fresh session window. Aborts if a late (post-reset) wake fired it; plans once per week (state tracked in `c4po/.claude/scheduled/state/`). |
...
| `/backlog-burndown` | c4po | Runs the `c4po-backlog-burndown` logic interactively — prioritizes and commits every backlog for real, presents the consolidated plan in this session instead of email, and implements items with the user in the loop (confirms before starting, stops on request). Skips the weekly gate, the pre-reset window check, and the state/plan file writes so it never blocks the scheduled run. |
...
| `/vet-skill` | c4po | Security pre-install audit of a third-party Claude Code skill or plugin (`$ARGUMENTS` = its name, repo, or local path). Ascertains the canonical upstream source, audits what it executes and reads, bundled scripts, `hooks/`, and credential footprint, runs a read-only dry-run, then recommends install / install-with-changes / decline. Not tied to a scheduled task. |
