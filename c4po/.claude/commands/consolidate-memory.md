...
2. In the **REPORT** phase — do NOT pipe to `notify-email.sh`. Output the digest
   directly into this session: what was examined and what changed, including
   "no changes" for an already-tidy store, so the run is legible. (SCOPE and CONSOLIDATE still run for real — the
